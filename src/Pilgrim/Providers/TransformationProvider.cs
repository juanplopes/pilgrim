...
using Pilgrim.Framework.Loggers;
using System.Data.Common;

namespace Pilgrim.Providers
{
    /// <summary>
    ...
    public abstract class TransformationProvider : ITransformationProvider
    {
        ...
        protected TransformationProvider(Dialect dialect, string invariantProvider, string connectionString)
        {
            _dialect = dialect;
            _connectionString = connectionString;
            _logger = new Logger(false);

            _connection = DbProviderFactories.GetFactory(invariantProvider).CreateConnection();
        ...
        public virtual IDataReader Select(string what, string from, string where, string orderBy)
        {
            return ExecuteQuery(String.Format("SELECT {0} FROM {1} WHERE {2} ORDER BY {3}", what, from, where, orderBy));
        }
        ...
        public virtual object SelectScalar(string what, string from, string where, string orderBy)
        {
            return ExecuteScalar(String.Format("SELECT {0} FROM {1} WHERE {2} ORDER BY {3}", what, from, where, orderBy));
        }
        ...
        {
            CreateSchemaInfoTable();
            Insert(_schemainfoname, new string[] { "version" }, new string[] { version.ToString() });
        ...
        {
            CreateSchemaInfoTable();
            Delete(_schemainfoname, "version", version.ToString());
        ...
        public void SchemaInfoName(string schemainfoname)
        {
