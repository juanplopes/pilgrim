...
namespace Pilgrim.Migrator
{
    public abstract class BaseMigrate
    {
        ...
        protected BaseMigrate(List<long> availableMigrations, ITransformationProvider provider, ILogger logger, string schemainfoname)
        {
            _provider = provider;
            _provider.SchemaInfoName(schemainfoname);
            _availableMigrations = availableMigrations;
            _original = new List<long> (_provider.AppliedMigrations.ToArray()); //clone
            _logger = logger;
        }

        public static BaseMigrate GetInstance(List<long> availableMigrations, ITransformationProvider provider, ILogger logger, string schemainfoname)
        {
            return new MigrateAnywhere(availableMigrations, provider, logger, schemainfoname);
        }
        ...
        public abstract void Migrate(IMigration migration, string schemainfoname);
